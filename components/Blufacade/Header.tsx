...
import { Menu, X, ChevronDown, ChevronLeft, ChevronRight, Instagram, Linkedin, Facebook, Twitter } from "lucide-react"
...
export function Header() {
  ...
  const megaProducts = (products && products.length > 0 ? products : []).map((p: any) => ({
    name: p.productName || p.name,
    image: p.image,
    slug: p.slug,
    category: p.category,
  }))
  const displayProducts = megaProducts.length > 0
    ? megaProducts
    : FALLBACK_PRODUCTS.map((p) => ({ name: p.name, image: p.image, slug: p.slug, category: p.category }))

  const megaServices = (services && services.length > 0 ? services : []).map((s: any) => ({
    name: s.serviceName || s.name,
    image: s.image,
    slug: s.slug,
  }))
  const displayServices = megaServices.length > 0
    ? megaServices
    : FALLBACK_SERVICES.map((s) => ({ name: s.serviceName, image: s.image, slug: s.slug }))

  // Get unique categories for products
  const productCategories = ["ALL", ...Array.from(new Set(displayProducts.map((p) => p.category).filter(Boolean)))] as string[]
        ...
        <div className="flex items-stretch h-14 sm:h-16 lg:h-[72px]">
          ...
          >
            <Link href="/" className="flex items-center gap-2 sm:gap-2.5 lg:gap-3.5 group">
              ...
              >
                <Image
                  src={settings?.logo || "/images/rayzor/logo/Rayzor Final Logo File-03.png"}
              ...
              <span
                className="text-base sm:text-lg md:text-xl lg:text-xl xl:text-2xl font-heading font-extrabold tracking-tight leading-none transition-colors duration-500 uppercase"
              >
                {(() => {
                  ...
                  if (idx > 0) {
                    return (
                      <>
                        <span style={{ color: useLightText ? "var(--brand-white)" : "var(--brand-dark)" }}>{upperName.slice(0, idx)}</span>
                        <span className="text-[var(--brand-blue)]">{upperName.slice(idx)}</span>
                      </>
                    );
                  }
                  return <span style={{ color: useLightText ? "var(--brand-white)" : "var(--brand-dark)" }}>{upperName}</span>;
                })()}
              </span>
            </Link>
          </div>
          ...
          <div className="hidden lg:flex flex-1 items-stretch bg-white">
            <nav className="flex items-center gap-4 xl:gap-7 h-full flex-1 justify-end pr-4 xl:pr-6">
                    ...
                    >
                      <Link
                        ...
                        className={`flex items-center gap-0.5 text-[10px] lg:text-[11px] xl:text-[13px] font-semibold uppercase tracking-wider whitespace-nowrap transition-colors ${
                          activeDropdown === item.label || pathname === item.href || pathname.startsWith(item.href + "/")
                            ? "text-[var(--brand-dark)]"
              ...
              {/* Download Profile */}
              <Link href="/about" className="flex items-center gap-1 text-[9px] lg:text-[10px] xl:text-[12px] font-semibold uppercase tracking-wider text-[var(--brand-dark)] border border-[var(--brand-dark)] px-2.5 lg:px-3 xl:px-4 py-1.5 xl:py-2 whitespace-nowrap hover:bg-gray-50 transition-colors">
            ...
            {/* Contact Us */}
            <Link href="/contact" className="flex items-center justify-center bg-[var(--brand-blue)] text-white px-4 lg:px-5 xl:px-8 h-full font-bold text-[10px] lg:text-[11px] xl:text-[13px] uppercase tracking-wider whitespace-nowrap hover:bg-[#3aa0cc] transition-colors">
              Contact Us
            </Link>
          </div>
              ...
              <div className="max-w-[1400px] mx-auto px-10 xl:px-16">
                ...
                {activeDropdown === "PRODUCTS" && (
                  <div className="pt-6 pb-6">
                    ...
                    {/* Carousel */}
                    <MegaCarousel items={filteredProducts} basePath="/products" />
                  </div>
                )}
                ...
                {activeDropdown === "SERVICES" && (
                  <div className="pt-6 pb-6">
                    ...
                    {/* Carousel */}
                    <MegaCarousel items={displayServices} basePath="/services" />
                  </div>
                )}
